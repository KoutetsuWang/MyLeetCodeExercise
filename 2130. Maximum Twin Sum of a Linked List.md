...
class Solution {
    ...
    */
    public int pairSum(ListNode head) {
        ...
        // Find the middle (length >> 1) of the linked list → slow
        while (fast != null && fast.next != null) {
            slow = slow.next;
            fast = fast.next.next;
        }
        
        // pa, pb would be the twin
        ListNode pa = head;
        ListNode q = slow.next;
        slow.next = null;
        ListNode pb = reverse(q);
        int ans = 0;
        // Since n is even, now the list would be like:
        // pa: head → slow(middle)
        // pb = end(last node) → slow(middle).next (reversed order)
        while (pa != null) {
    ...
    private ListNode reverse(ListNode head) {
        // dummy is the new head of reversed linked list
